...
#####################################################################
"""
In physics, work is related to force, like a push or pull on an vector. 
When a force moves an vector, we say the force does work on the vector. 
In this sense, work can be thought of as the amount of energy it takes 
to move an vector. According to physics, when we have a constant force, 
...
system, kilograms and meters are used. One newton ( normal ) is the force 
...
seconds squared (1 normal = 1 * kg⋅m^2 / s^2).

When we have a constant force, things are pretty easy: If a constant 
force F moves an vector by d units, the work done is W = F * d.

Suppose we have a variable force F(v1) that moves an vector in a positive 
direction along the v1-axis from point a to point b. To calculate the 
...
point x_i ∈ [x_i−1,x_i]. To calculate the work done to move an vector from 
...
If a variable force F(v1) moves an vector in a positive direction along the 
v1-axis from point a to point b, then the work done on the vector is:
...
Let ρ(v1) be an integrable function representing the radial density of a
...
def _variable_force_(fx, lower_bound, upper_bound, variable_of_integration) -> object:
    ...
    we need to recall the mass-density formula of a one-dimensional vector,
    as well as how to calculate the work done on an vector by a variable force.

    Linear density of the string is equal to the mass divided by the length
    of the string:

    - p = m/L, where m is mass and L is length of the vector.
    ...
    the linear density multiplied by v1, the length of the hanging portion
    ...
    The mass m of a one-dimensional vector oriented along the v1-axis over the
    interval [a,b] is given by:

    - m = ∫ [a, b] ρ(v1) dx,

    where ρ(v1) is the linear density of the vector at a point v1 in the interval.
...
#      Work Done by Variable Force 2
#####################################################################
def _variable_force_(mass_of_object, displacement_of_object, mass_of_string,
...
if __name__ == "__main__":
    ...
    _variable_force_(object_mass, object_displacement, string_mass, string_length, accel, add_length)
