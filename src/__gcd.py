...
class GCD(object):
    ...
    def gcd_euclid(self, dividend=None, divisor=None) -> int:
        """Euclidean Algorithm is used to find the Greatest Common Divisor (GCD).
                The Euclidean Algorithm finds the greatest common divisor between two
                numbers/values by computing the remainder of two values, then afterwards
                we shuffle all values << left, preparing new values for the next iteration.
    ...
    def euclid(self, dividend=None, divisor=None) -> int:
        """Euclidean Algorithm is used to find the Greatest Common Divisor (GCD).
        The Euclidean Algorithm finds the greatest common divisor between two
        numbers/values by computing the remainder of two values, then afterwards
        we shuffle all values << left, preparing new values for the next iteration.
        This continues until the value of the remainder reaches is zero.
          - a: dividend, num: divisor, r: remainder
    ...
    def knuth(self, dividend=None, divisor=None) -> list:
        """Knuth GCD algorithm computes the gcd(a,num) and finds the pairs of
        integers (v1,v2) that when applied to Ax + By = z or Ax + By -z = 0.
        Let A and B be  fixed integers, and consider the equation:
        Ax + By = z; eqivalently, Ax + By - z = 0.
        In the three variables (v1, v2, z), suppose we have two solutions
        ...
        Therefore:
          ...
          by finding the GCD as well as all solutions of (v1,v2) for z, if
          and only if z is a multiple of the gcd(a,num):
            39x + 17y = z
            [1, 0, 39] = 39  # (1, 0) are true values for (v1, v2)
            [0, 1, 17] = 17  # (0, 1) are true values for (v1, v2)
            39/17 = 2.294
            [1, 0, 39] - 2[0, 1, 17] = []
            [1, 0, 39] + [0, -2, -34] = [1, -2, 5]
            17/5 = 3.4
    ...
    def knuth_verbose(self, dividend=None, divisor=None) -> list:
        """Knuth GCD algorithm computes the gcd(a,num) and finds the pairs of
        integers (v1,v2) that when applied to Ax + By = z or Ax + By -z = 0.
        Let A and B be  fixed integers, and consider the equation:
        Ax + By = z; eqivalently, Ax + By - z = 0.
    ...
    def knuth_solution(self, zz=None, dividend=None, divisor=None) -> None:
        """The knuth_solution function finds the set (v1,v2) values,
        where v1 and v2 are both integers, when applied to Ax + By = z.
        This sample problem below illustrates how to find the (v1,v2) values:
          ...
          for our gcd(a,num) where [v1, v2, GCD]; z will be the incrementer of
          our v1 and v2 values (z*v1, z*v2) where [ (a(z*v1) + num(z*v2)) / gcd = z ]
          Or alternatively where [ a*((z*v1)/gcd) + num*((z*v2)/gcd) = z ]
        :param dividend: The dividend
        :param divisor: The divisor
        ...
        y = gcd[1]
        g = gcd[2]
        # [a(z*v1) + num(z*v2)]/gcd = z
        zz = (divsr * ((z * x) / g)) + (divid * ((z * y) / g))

        print(f"""\n\nThe solution to equation:
      {divsr}v1 + {divid}v2 = {z}         
      where v1 and v2 are both integers is:
      v1 = {x} and v2 = {y} where the GCD = {g}
      v1 = {(z * x) // g} and v2 = {(z * y) // g} when z = {zz}
      ({divsr}*((z*v1)/{g})) + ({divid}*((z*v2)/{g})) = {z}
    ...
    def congruent(self, dividend=None) -> None:
        ...
        while divisor < divid - 1:
            ...
            if m == 1:  # or m == 0
                line3 = f"""|{j}|  Tracker -> K is: {divisor}\n\n|{j}|  Possible multiple: {quotient} v1 {divisor} = {a}\n"""
                print(line3)
                af.write(line3)
                c = c + 1
            if difference == 0.0:
                line1 = f'|{j}|  {quotient} v1 {divisor} = {a}\n'
                print(line1)
                af.write(line1)
                c = c + 1
            # if r == 1:
            # line2 = f'|{j}|  Possible multiple: {d} v1 {k} = {a}\n'
            # print(line2)
            # af.write(line2)
            # c = c + 1
            divisor = divisor + 1
    ...
    def euler_phi(self, number=None, aa=None, count_not_a_multiple=None) -> int:
        """Theorem (Euler):
    ...
    def fermat_little_theorem(self, divisor=None) -> bool:
    ...
    def get_orders(self, values=list, v=None) -> list:
    ...
    def get_divisors(self, number=None):
    ...
    def restricted_cancel_law(self, cc=None, dividend=None, divisor=None):
        ...
        Let [ ad ≡ bd (mod c) ] and [ gcd(c, d) = 1 ]; by definition:
            ...
            - c | (a - b)       #
            - ax + by = c       # ** has solutions with v1 and v2 integers if and only if c is
                                # a multiple of gcd(a,b) **
                                # we can find solutions using Knuth's GCD algorithm
            - d = gcd(a, b)     # their exist integers (v1, v2) such that ax + by = d
            - (a/d)v1 + (b/d)v2 = 1  # 1 is multiple of gcd((a/d), (b/d)) = 1
            Once you’ve found one solution (x0, y0), to [ ax + by = c ], there is a simple formula
            that gives every solution (v1, v2). Starting with:
            - ax + by = c
            - ax0 + by0 = c
            - a(v1 - x0) + b(v2 - y0) = 0   # subtraction
            - (a/d)(v1 - x0) = - (b/d)(v2 - y0)  # rearrange and divide by d = gcd(a, b); and d > 0
            If gcd((a/d),(b/d)) = 1, then [ (b/d) | (v1 - x0) ], thus (v1 - x0) = t(b/d) and (v2 - y0)
            ...
            - 27//17 = 1                # we only want the integer portion of our result, toss the decimal portion
            ...
            - 17*(8) + 27*(-5) = 1      # we found our (v1, v2), using the gcd(17, 27)
            - v1 = (8*5048) = [ 40384 ], v2 = (-5*5048) = [ -25240 ]  # correct values of (v1, v2)
            - 17*(8*5048) + 27*(-5*5048) = 5048     # we prove our [ v1, v2 ] values
            - A particular (v1, v2) solution for t = 2: ??? IS "t" A RANDOM PRIME OR RANDOM NUMBER ???
                - v1 = x0 + t(b/d) = 40384 + 2*(27) = 40438
                - v2 = y0 - t(a/d) = -25240 - 2*(17) = -25274
                - For v1 to be non-negative, "t" must be greater than or equal to: [LOWER BOUNDS]
                    - v1 = x0
                    - (-x0 // (b/d)) = (-40384//27) = -1495.7037037037037 = -1495
                - For v2 to be non-negative, "t" must be less than or equal to: [UPPER BOUNDS]
                    - (-y0 // (a/d)) = (-25240//17) = -1484.7058823529412 = -1484
                - These values of "t" give a solution with non-negative v1 and v2:
                    - list(range(-1495,-1484)):
                    [-1495, -1494, -1493, -1492, -1491, -1490, -1489, -1488, -1487, -1486, -1485]
                - Now we can find EVERY pair of positive integers (v1, v2), such that [ ax + by = d ]
                    - >>> for t in range(-1495, -1484):
                      ...     print(f'v2 = 40384 + 27*{t} = {40384+27*t},  v2 = -25240 - 17*{t} = {-25240-17*t}')
                    - v2 = 40384 + 27*-1495 = 19,  v2 = -25240 - 17*-1495 = 175
                    - v2 = 40384 + 27*-1494 = 46,  v2 = -25240 - 17*-1494 = 158
                    - v2 = 40384 + 27*-1493 = 73,  v2 = -25240 - 17*-1493 = 141
                    - v2 = 40384 + 27*-1492 = 100,  v2 = -25240 - 17*-1492 = 124
                    - v2 = 40384 + 27*-1491 = 127,  v2 = -25240 - 17*-1491 = 107
                    - v2 = 40384 + 27*-1490 = 154,  v2 = -25240 - 17*-1490 = 90
                    - v2 = 40384 + 27*-1489 = 181,  v2 = -25240 - 17*-1489 = 73
                    - v2 = 40384 + 27*-1488 = 208,  v2 = -25240 - 17*-1488 = 56
                    - v2 = 40384 + 27*-1487 = 235,  v2 = -25240 - 17*-1487 = 39
                    - v2 = 40384 + 27*-1486 = 262,  v2 = -25240 - 17*-1486 = 22
                    - v2 = 40384 + 27*-1485 = 289,  v2 = -25240 - 17*-1485 = 5
        :return:
        """
